...
- 唯一 AP：`LensCap-<设备ID>`，默认密码 `telescope`
- 控制页面：`http://192.168.4.1/`，支持盖板控制和平场灯 1–255 亮度调节
- 配置页面：`http://192.168.4.1/config`，可配置 STA、开盖位置和关盖位置，参数保存到 EEPROM
- DLC TCP 服务：端口 `4030`，协议与 USB 串口相同，命令使用 `<`、`>` 封装
- AP 在 STA 模式下保持启用，便于连接失败时恢复配置

ASCOM 使用 USB 时选择 `Serial` 和 115200 baud；使用 WiFi 时选择 `TCP`，填写配置页显示的 STA IP 和端口 `4030`。
...
### NodeMCU ESP-12F 接线（CH340 版本）

本固件已在 NodeMCU ESP-12F 开发板上验证。该板上标记为 `VU` 的针脚是 USB 5V 电源，外观上容易与 `VV` 混淆。

| 舵机线 | NodeMCU 针脚 | 说明 |
|--------|--------------|------|
| 白 / 橙 / 黄 | `D2`（GPIO4） | 控制信号 |
| 红 | `VU` | USB 5V；适用于 SG90 等小型舵机 |
| 黑 / 棕 | `G` | 接地 |

`D2` 位于下排、靠右侧第三个 I/O 针脚。`3V` 针脚不用于舵机供电。

对于 MG90、MG996R 等电流较大的舵机，将红线连接到独立的稳定 5V 电源；外部电源 GND 与 NodeMCU 的 `G` 必须共地。建议在舵机电源线附近并联一个 **470–1000 µF、耐压至少 6.3V** 的电解电容，电容正极接 5V、负极接 GND；可另加 0.1 µF 陶瓷电容抑制高频干扰。
...
2. **选择开发板**  
   - Wemos D1 Mini：工具 → 开发板 → ESP8266 → **LOLIN(WEMOS) D1 R2 & mini**
   - NodeMCU ESP-12F：工具 → 开发板 → ESP8266 → **NodeMCU 1.0 (ESP-12E Module)**
...
1. 上电后 ESP8266 创建热点 `LensCap-<设备ID>`（密码 `telescope`）
2. 手机连接该 WiFi
3. 浏览器打开 **http://192.168.4.1**
4. 点击按钮开关盖子

设备 ID 取自 ESP8266 芯片 ID 的十六进制后六位。例如，芯片 ID 为 `513670` 时，热点名称为 `LensCap-513670`。
