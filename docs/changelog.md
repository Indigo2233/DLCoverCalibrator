...
## 2026-08 — ASCOM 驱动 1.2.0 串口恢复

### 问题

CH340 串口在远程运行中偶发打开成功但设备无响应。原有定时轮询允许回调重入，单次命令超时期间可能持续积压后续轮询；断开操作也会等待这些回调释放串口。

### 改动

- 握手采用有界恢复：首次无响应后执行 DTR 复位，释放旧 `SerialPort` 实例并重建串口
- 禁止状态轮询回调重入，并在首个无响应命令后结束本轮轮询
- 连续两轮无响应后自动停止轮询、恢复连接并重新读取设备状态
- 新增 ASCOM `ResetDevice` Action，用于手动执行同一恢复流程
- 连接失败重试次数固定为两轮，避免长时间无上限阻塞
- 从 ASCOM 驱动和 Setup 界面移除硬件未配备的副舵机设置及相关命令
- 增加串口恢复、轮询互斥和自动恢复回归测试
